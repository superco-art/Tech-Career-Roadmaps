...
const NodeDetail: React.FC<NodeDetailProps> = ({ subNode, onClose, isDarkMode }) => {
  const getDifficultyColor = (difficulty: string) => {
    switch (difficulty) {
      case 'Beginner': return isDarkMode ? 'bg-soft-green bg-opacity-20 text-soft-green border border-soft-green border-opacity-50' : 'bg-green-100 text-green-800';
      case 'Intermediate': return isDarkMode ? 'bg-soft-yellow bg-opacity-20 text-soft-yellow border border-soft-yellow border-opacity-50' : 'bg-yellow-100 text-yellow-800';
      case 'Advanced': return isDarkMode ? 'bg-soft-red bg-opacity-20 text-soft-red border border-soft-red border-opacity-50' : 'bg-red-100 text-red-800';
  ...
  return (
    <div className={`fixed inset-0 flex items-center justify-center z-50 p-4 ${
      ...
      <div className={`rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto transition-all duration-300 ${
        isDarkMode 
          ? 'bg-dark-surface shadow-2xl shadow-soft-blue/20 border border-dark-border' 
          : 'bg-white shadow-2xl'
      }`}>
        {/* Header */}
          ...
          <div className="flex items-start justify-between">
            ...
            <button
              ...
              className={`p-2 rounded-full transition-all duration-200 ${
                isDarkMode 
                  ? 'hover:bg-dark-card hover:shadow-soft-sm text-dark-muted hover:text-soft-blue' 
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center space-x-2">
              <Target className={`w-5 h-5 ${isDarkMode ? 'text-soft-blue' : 'text-blue-500'}`} />
            ...
            {subNode.estimatedTime && (
              <div className="flex items-center space-x-2">
                <Clock className={`w-5 h-5 ${isDarkMode ? 'text-soft-green' : 'text-green-500'}`} />
          ...
          {subNode.prerequisites && subNode.prerequisites.length > 0 && (
            <div>
              ...
              <div className={`rounded-lg p-4 border transition-all duration-300 ${
                isDarkMode 
                  ? 'bg-soft-orange bg-opacity-20 border-soft-orange border-opacity-50' 
                  ...
                  {subNode.prerequisites.map((prereq, index) => (
                    <li key={index} className="flex items-center space-x-2">
                      <span className={`w-2 h-2 rounded-full ${
                        isDarkMode ? 'bg-soft-orange' : 'bg-orange-400'
                      }`}></span>
                      <span className={isDarkMode ? 'text-soft-orange' : 'text-orange-800'}>{prereq}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {subNode.skills.map((skill, index) => (
                <div key={index} className={`flex items-center space-x-2 rounded-lg p-3 border transition-all duration-300 ${
                  isDarkMode 
                    ? 'bg-soft-blue bg-opacity-20 border-soft-blue border-opacity-50' 
                    : 'bg-blue-50 border-blue-200'
                }`}>
                  <span className={`w-2 h-2 rounded-full ${
                    isDarkMode ? 'bg-soft-blue' : 'bg-blue-500'
                  }`}></span>
                  <span className={`font-medium ${
                    isDarkMode ? 'text-soft-blue' : 'text-blue-800'
                  }`}>{skill}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {subNode.resources.map((resource, index) => (
                <div key={index} className={`border rounded-lg p-4 transition-all duration-200 ${
                  isDarkMode 
                    ? 'border-dark-border hover:border-soft-purple hover:shadow-soft-sm bg-dark-card' 
                  ...
                  <div className="flex items-start justify-between">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-2 mt-2">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            resource.free 
                              ? isDarkMode 
                                ? 'bg-soft-green bg-opacity-20 text-soft-green border border-soft-green border-opacity-50' 
                                : 'bg-green-100 text-green-800'
                              : isDarkMode 
                                ? 'bg-soft-orange bg-opacity-20 text-soft-orange border border-soft-orange border-opacity-50' 
                    ...
                    >
                      <ExternalLink className={`w-5 h-5 cursor-pointer transition-colors duration-200 ${
                        isDarkMode 
                          ? 'text-dark-muted hover:text-soft-blue' 
